...
- **Priority: A (core) > C (graphics) > D (Windows) > G (feeds D) > H (Android) > K (feeds E) > F > E > I > B > T (Wi-Fi, low).** Track U (console, W41+) is niche and read-heavy — schedule when platforms interest you, never in a crowded week. Pick one mobile platform primary; the other goes secondary (re-run later).
...
| 41+ | — | Console security (Track U, niche, optional) |
| 47+ | Browser → chains | — |
| post-M23 | — | Career: portfolio/disclosure/interviews (Track O) |

**Total:** ~18 months focused / ~25 moderate to jailbreak+root literate (+ GPU/Windows/driver/Android/rootkit/hooking/crypto/DFIR tracks). Browser: open-ended. Phase 23 (career) runs after — the plan ends when skills convert, not when the labs do. Make-Your-Own (Phase 24, Track P) is optional W46+ — run only if other tracks are ahead (stretch adds: own C compiler 09, own RISC-V microkernel on your 02-11 emulator 10, own SQLite file format 11, own SSH server 12, own container runtime 13, own terminal emulator 14, own filesystem 15, own memory allocator 16, own HTTP server 17, own WireGuard-lite 18, own DNS resolver 19, own packet sniffer/crafter 20, own BGP speaker 21, own onion router 22, own QUIC-lite 23, own NAT/router 24, own compression 25, own LSM-tree 26, own PNG decoder 27, own quantum simulator 28, own Kademlia DHT 29, own sampling profiler 30, own bootloader 31, own NTFS parser 32, own APFS parser 33, own mail server 34, own JPEG decoder 35, own BitTorrent client 36, own chess engine 37, own text editor 38, own package manager 39, own audio synth 40, own search engine 41, own key-value cache 42, systems gauntlet 43, own TPM lab 44, own registry parser 45, own MQTT broker 46, own JS interpreter 47, own orchestrator 48, own font renderer 49, own logic analyzer 50, own time-series DB 51, own message queue 52, own Tailscale-lite 53, own video container 54, own 3D model loader 55, own network scanner 56, own IDS engine 57, own object storage 58, own QR codec 59, own spam filter 60, own p2p sync 61, own SOCKS proxy 62, own raycaster 63, own HD wallet 64, own audio modem 65, own load balancer 66, own ad-blocking DNS 67, own terminal multiplexer 68, own WebAuthn authenticator 69, own CA 70, own encrypted overlay FS 71, own spreadsheet 72, own SVG renderer 73, own CI runner 74, own package registry 75, own video codec-lite 76, own VoIP 77, own container image builder 78, own malware ML detector 79, own packet replayer 80, own unikernel 81, own kernel gdb stub 82, own backup tool 83, own init system 84, own config manager 85, own htop-lite 86, own linker 87, own LDAP-lite 88, own TTS 89, own rate limiter 90, own supply-chain signer 91, own FFT visualizer 92, own Sokoban solver 93, own PDF reader 94, own IMAP client 95, own DNSSEC-lite 96, own Mach-O parser 97, own MP4 muxer 98, own DMG reader 99, own audio editor 100, own video editor-lite 101, own HLS streamer 102, own macOS process monitor 103, own dylib injector 104, own logarchive parser 105, own chiptune sequencer 106, own subtitle engine 107, own DASH player 108, own plist parser 109, own arm64 playground 110, own Metal compute 111, own game cheat tool 112, own noise reduction 113, own image stabilizer 114, own iOS app analyzer 115, own BLE scanner 116, own Game Boy emulator 117, own io_uring-lite 118, own ETW consumer 119). Track Q (privesc, W20+) and Track R (BT devices, W36+) are parallel add-ons — defer under the >2-concurrent-tracks rule.
...
- [ ] M29: console platform matrix + one re-created chain-class, defense synthesis (W52+, Track U, optional)
