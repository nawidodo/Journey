...
Zero GPU knowledge assumed. You need: a Mac, Xcode (free, App Store), and the **C basics from 00-01** (that's the only prerequisite). Each unit ends with something you **run and see** — no unit passes until its verification line is true AND its lesson check is answered in writing (`notes/uN-quiz.md`). ~1–2h per unit, 10 lessons + 1 capstone ≈ 3–4 weeks. When done, jump into gate S4 of [`GPU-PATH.md`](GPU-PATH.md).

Mental model to carry (re-read when stuck): **a GPU is a calculator with thousands of tiny cores.** The CPU gives each core a job by *index* (`you are thread 7, do row 7`); all cores run the SAME program (a *kernel*) on different data. That's why GPUs win at "do the same math on a huge array" — you pay latency once, then reap bandwidth. Everything below teaches one skill: **write the kernel, shape the data, measure the speed.**

**Rule for every lesson check:** answer in your own words, one paragraph max, saved to `notes/uN-quiz.md`. If you can't answer without looking at the unit, go back and re-run it. The check is the gate — it's how the course keeps you honest.
...
**Lesson check (write):** name the 6 words and give each a one-line definition in YOUR wording.
...
**Lesson check (write):** walk the 7 steps of the numbered comments aloud-to-paper. Then answer: why did changing n to 1,000,000 not change the kernel code?
...
**Lesson check (write):** draw the grid→threadgroup→thread nesting and mark where `thread_position_in_grid` fits. Answer: what would `thread_position_in_threadgroup` mean?
...
**Lesson check (write):** given array of 1M floats and 1M threads, write the access pattern that is coalesced vs one that is not, with a sentence on the mechanism (32-at-a-time reads).
...
**Lesson check (write):** why does sharing data through threadgroup memory beat reading buffers repeatedly? Name the two costs it avoids.
...
**Lesson check (write):** describe the journey of one triangle: which stages run per-vertex, which run per-pixel, and what the rasterizer does between them.
...
**Lesson check (write):** how does the sampler change the color you get at a non-integer coordinate? Show the 2-x-2 quad for linear sampling.
...
**Lesson check (write):** your shader is a function pixel-position → color. Identify where in the fragment shader you read the texture, and where you modify it — quote your own lines.
...
**Lesson check (write):** interpret the HUD: what two numbers tell you if you're GPU-bound vs fill-rate-bound? What symptom sends you back to U3's coalescing?
...
**Lesson check (write):** why does matrix multiply read each A row B columns repeatedly — and how does the U4 threadgroup tile change how many times the GPU touches memory?
...
**Lesson check (write):** from memory, list the 6 words plus each unit that taught it — the checklist of what you own.

---

## U11 — CAPSTONE: close the notes, ship a GPU project
Prereq: U0–U10 passed (verifications true + quiz answers in `notes/`). **Close all notes and the docs.** Choose ONE:
- **(a) Emulator shader pack**: three shaders (scanline/CRT, LCD-grid for the Game Boy, upscale+sharpen for NES) toggleable live on your running core, with a HUD-verified frame budget under your target.
- **(b) Compute tool**: your U9 matrix multiply turned into a usable library (any size, library file, edge-case sizes error-checked) with the CPU-vs-GPU table published — the shape of an inference engine.
- **(c) Graphics toy**: U5 triangle grown to a rotating cube with texture + one lighting trick, 60 fps verified by HUD.

Build it with tutorials closed; when stuck, re-run the *unit verification* that covers the stuck area — going back is part of the method. Finish with:
`labs/gpu-capstone.md`: what you built, the HUD/benchmark evidence, the one thing you'd ship differently next, and a 3-line self-audit against U10's checklist.
**Pass = your project runs and you can explain every line of the shader/kernel in it, cold.**

## Rules
1. No unit skip: verification true + lesson check written (own words) before the next unit.
2. Copying allowed ONLY in U5 (the triangle boilerplate) — everywhere else, write from concept; erase-and-retry once when stuck (the 24-43 systems-gauntlet method).
3. Timebox each unit 2h; stuck past that = go back one unit and re-run its verification.
4. Own machine only; all examples on your Mac's GPU via Metal.
5. If your emulator core isn't ready at U7: do the PNG-frame static version and return — don't stall the course on the core.
6. The capstone (U11) IS the graduation: no GPU-PATH S4 entry until its pass line is true.