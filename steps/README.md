...
| `08-Browser-Exploitation` | JS engines → d8 CVE → JSC/WebKit (04, optional) + own JS-engine exploit (05) | 47+ | A |
| `09-Real-World-Chains` | ForcedEntry, Pegasus, Pwn2Own + own exploit chain (05) | capstone | A |
...
| `29-Console-Security` | trust-model matrix (boot/key/hardening/sandbox/entry surfaces) → PS4/PS5 Orbis FreeBSD chains (own-VM UAF re-creation) → Nintendo Switch Tegra/TrustZone/RCM (NCA parser + boot-verifier sim) → Xbox HV/Secure-Kernel (isolation lab) → capstone platform matrix + deep-dive + long-tail archive (retro/media-tray/3D-era/handheld/micro-PC classes, own Atari 2600 core) + odd-silicon era (PS2/PS3 Cell/Saturn/Dreamcast/Jaguar/Vita — SPE-style kernel lab) (M29) | 41+ opt | U |
...
Track U parallel (start W41+, niche; console platform security — pairs Phase 02 emulators + 24-01; own consoles lab-only; read-heavy with own-VM/parser labs): `29`
