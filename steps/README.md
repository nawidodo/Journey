...
| `29-Console-Security` | trust-model matrix (boot/key/hardening/sandbox/entry surfaces) → PS4/PS5 Orbis FreeBSD chains (own-VM UAF re-creation) → Nintendo Switch Tegra/TrustZone/RCM (NCA parser + boot-verifier sim) → Xbox HV/Secure-Kernel (isolation lab) → capstone platform matrix + deep-dive (M29) | 41+ opt | U |
