...
| `10-GPU-Compute` | concepts → CPU SIMD → Metal compute → memory → software renderer → Metal graphics → shaders (MSL/GLSL/HLSL) → textures → async → profiling → Vulkan/DX12 → capstone (+ path tracer 13 + CUDA for AI 14 + own NN 15, stretch) | 12–28 | C |
